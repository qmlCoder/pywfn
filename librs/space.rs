...
// 计算波函数
// xyz:以原子为中心的格点坐标
fn gtf(xyz: &[f64; 3], lmn: &[i32; 3], alp: f64, level: u32) -> (f64, [f64; 3], [[f64; 3]; 3]) {
    let x = xyz[0];
    let y = xyz[1];
    let z = xyz[2];
    let l = lmn[0];
    let m = lmn[1];
    let n = lmn[2];
    let facs = [1., 1., 3., 15., 105., 945.]; //2n-1的双阶乘
...
fn ato_wfn(
    xyz: &[f64; 3],       // 空间中任意一点的坐标
    xyzs: &Vec<[f64; 3]>, // 原子轨道坐标；基函数坐标
    lmns: &Vec<[i32; 3]>,
    coes: &Vec<Vec<f64>>,
    alps: &Vec<Vec<f64>>,
    level: u32,
    ...
    for obt in 0..nobt {
        ...
        if level >= 2 {
            for k in 0..3 {
                for l in 0..3 {
                    mol_rho2[k][l] +=
                        2.0 * obt_wfn1[k] * obt_wfn1[l] + 2.0 * obt_wfn0 * obt_wfn2[k][l];
                }
            }
        }
        obt_wfn0 = 0.0;
    ...
    for i in 0..nato {
        for j in 0..nato {
            ...
            if level >= 1 {
                for k in 0..3 {
                    ato_rho1[i][k] +=
                        (ato_wfn0[i] * ato_wfn1[j][k] + ato_wfn0[j] * ato_wfn1[i][k]) * matp[i][j];
                }
            }
            if level >= 2 {
                for k in 0..3 {
                    for l in 0..3 {
                        ato_rho2[i][k][l] += (ato_wfn2[i][k][l] * ato_wfn0[j]
                            + ato_wfn1[i][k] * ato_wfn1[j][l]
                            + ato_wfn1[i][l] * ato_wfn1[j][k]
                            + ato_wfn0[i] * ato_wfn2[j][k][l])
                            * matp[i][j];
                    }
                }
            }
...
// 计算原子(片段)间静电相互作用能 electrostatic interaction energy between atoms
pub fn calc_EIEBA(
    fragA: &Vec<[f64; 4]>,  // 片段A的原子(核电荷数及坐标)
    fragB: &Vec<[f64; 4]>,  // 片段B的原子(核电荷数及坐标)
    rhoA: &Vec<f64>,        // 片段A的电子密度
    rhoB: &Vec<f64>,        //片段B的电子密度
    gridsA: &Vec<[f64; 3]>, // 格点坐标
    gridsB: &Vec<[f64; 3]>, // 格点坐标
    weitsA: &Vec<f64>,      // 格点权重
    weitsB: &Vec<f64>,      // 格点权重
) -> (f64, f64, f64) {
    let mut eng_ee = 0.0; // 电子-电子排斥
    let mut eng_en = 0.0; // 核-电子吸引
    let mut eng_nn = 0.0;

    for i in 0..rhoA.len() {
        // 电子-电子排斥
        for j in 0..rhoB.len() {
            let dist_ee = calc_dist(&gridsA[i], &gridsB[j]); // 两个格点之间的距离
            if dist_ee < 1e-6 {
                continue;
            }
            eng_ee += rhoA[i] * weitsA[i] * rhoB[j] * weitsB[j] / dist_ee
        }
    }

    // A核-B电子 吸引
    for g in 0..rhoB.len() {
        for a in 0..fragA.len() {
            let [c, x, y, z] = fragA[a];
            let dist_ne = calc_dist(&[x, y, z], &gridsB[g]);
            if dist_ne < 1e-6 {
                continue;
            }
            eng_en -= rhoB[g] * weitsB[g] * c / dist_ne;
        }
    }

    // B核-A电子 吸引
    for g in 0..rhoA.len() {
        for a in 0..fragB.len() {
            let [c, x, y, z] = fragB[a];
            let dist_ne = calc_dist(&[x, y, z], &gridsA[g]);
            if dist_ne < 1e-6 {
                continue;
            }
            eng_en -= rhoA[g] * weitsA[g] * c / dist_ne;
        }
    }

    // A核-B核 排斥
    for i in 0..fragA.len() {
        let [ca, xa, ya, za] = fragA[i];
        for j in 0..fragB.len() {
            let [cb, xb, yb, zb] = fragB[j];
            let dist_ij = calc_dist(&[xa, ya, za], &[xb, yb, zb]);
            eng_nn += ca * cb / dist_ij;
        }
    }

    (eng_ee, eng_en, eng_nn)
}

#[pyfunction]
pub fn calc_EIEBA_rs(
    fragA: Vec<[f64; 4]>,  // 片段A的原子(核电荷数及坐标)
    fragB: Vec<[f64; 4]>,  // 片段B的原子(核电荷数及坐标)
    rhosA: Vec<f64>,       // 片段A的电子密度
    rhosB: Vec<f64>,       //片段B的电子密度
    gridsA: Vec<[f64; 3]>, // 格点坐标
    gridsB: Vec<[f64; 3]>, // 格点坐标
    weitsA: Vec<f64>,      // 格点权重
    weitsB: Vec<f64>,      // 格点权重
    ...
    calc_EIEBA(
        &fragA, &fragB, &rhosA, &rhosB, &gridsA, &gridsB, &weitsA, &weitsB,
    )
}
...
pub fn ato_wfns_rs(
    // 计算原子轨道波函数
    grids: Vec<[f64; 3]>, // 空间格点
    xyzs: Vec<[f64; 3]>,  // 原子轨道坐标
    lmns: Vec<[i32; 3]>,  // 原子轨道角动量
    coes: Vec<Vec<f64>>,  // 原子轨道系数
    alps: Vec<Vec<f64>>,  // 原子轨道指数
    level: u32,
...
pub fn mol_rhos_rs(
    ...
    // 拆分并行计算结果
    let (mut mol_rho0s, mut mol_rho1s, mut mol_rho2s) = (Vec::new(), Vec::new(), Vec::new());
