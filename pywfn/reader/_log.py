...
class LogReader(reader.Reader):
    def __init__(self, path:str,cache:bool=False) -> None:
        ...
        self.search_title()
        self._titles=rlib.search_title(self.path)
    ...
    def read_coords(self)->tuple[list[str],np.ndarray]|None:
        ...
        syms,xyzs=rlib.read_geome(self.path,self._titles["geome"])
        syms=[elements[int(e)].sym for e in syms]
    ...
    def read_basisData(self):
        ...
        # nbas=rlib.read_nbas(self.path,self._titles['nbas'])
        basisDatas=[]
        basis=rlib.read_basis(self.path,self._titles['basis'])
        for atm,shl,ang,alp,coe in basis:
            basisDatas.append(BasisData(atm,shl,ang,alp,coe))
        return basisDatas
    ...
    def read_CMs(self)->tuple[list,list,list,list,list,np.ndarray]:
        ...
        """
        nbas=rlib.read_nbas(self.path,self._titles["nbas"])
        if "cmat" in self._titles.keys():
            atms,shls,syms,engs,occs,cmat=rlib.read_cmat(self.path,self._titles["cmat"],nbas)
            CM=np.array(cmat)
        elif "cmat_a" in self._titles.keys() and "cmat_b" in self._titles.keys():
            atms_a,shls_a,syms_a,engs_a,occs_a,cmat_a=rlib.read_cmat(self.path,self._titles["cmat_a"],nbas)
            atms_b,shls_b,syms_b,engs_b,occs_b,cmat_b=rlib.read_cmat(self.path,self._titles["cmat_b"],nbas)
            CMa=np.array(cmat_a)
            CMb=np.array(cmat_b)
            ...
            raise ValueError('Invalid MO type')
